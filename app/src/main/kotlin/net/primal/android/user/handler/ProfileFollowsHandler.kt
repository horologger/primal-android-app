...
class ProfileFollowsHandler @Inject constructor(
    ...
    private fun setError(error: FollowResult) = scope.launch { results.emit(error) }

    init {
        observeActions()
    }

    fun follow(userId: String, profileId: String) =
        setAction(action = Action.Follow(userId = userId, profileId = profileId))

    fun unfollow(userId: String, profileId: String) =
    ...
    @FlowPreview
    private fun observeActions() = scope.launch {
        actions.batchOnInactivity(inactivityTimeout = 3.seconds)
            .collect { actions ->
    ...
    private suspend fun processActions(actions: List<Action>, forceUpdate: Boolean) {
        ...
        runCatching {
            ...
            setError(error = FollowResult.Error(actions = actions, error = error))
        }
    }
